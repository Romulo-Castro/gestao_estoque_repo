...
class BalanceSheetData {
  ...
  factory BalanceSheetData.fromDocuments(List<Document> documents) {
    ...
    for (final document in documents) {
      ...
      for (final item in document.items) {
        documentTotal += item.quantity * item.price;
      }

      final balanceItem = BalanceSheetItem(
      ...
      if (_isInflowDocument(document.type)) {
        inflowItems.add(balanceItem);
        totalInflows += documentTotal;
      } else if (_isOutflowDocument(document.type)) {
  ...
  static String _getDocumentDescription(Document document) {
    final typeStr = _getDocumentTypeDisplayName(document.type);
    if (document.notes != null && document.notes!.isNotEmpty) {
      return '$typeStr - ${document.notes!}';
    }
    return typeStr;
