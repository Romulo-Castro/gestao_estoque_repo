...
import '../../../shared/services/simple_report_service.dart';
...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Future<void> _generateStockReportPdf() async {
    try {
      ...
      // Ensure stock provider has the correct store ID and refresh stock items
      stockProvider.setStoreId(selectedStore.id);
      final items = stockProvider.items;
      
      if (items.isEmpty) {
        // Check if there's an error in fetching
        if (stockProvider.hasError) {
          ErrorHandler.showErrorSnackBar(context, 'Erro ao carregar itens: ${stockProvider.error}');
        } else {
          ErrorHandler.showErrorSnackBar(context, 'Nenhum item de estoque encontrado para esta loja');
        }
        return;
      }
  ...
  Future<void> _generateInventoryReportPdf() async {
    try {
      ...
      // Ensure stock provider has the correct store ID
      stockProvider.setStoreId(selectedStore.id);
      final items = stockProvider.items;
                              ...
                              return Column(
                                children: [
                                  ListTile(
                                    ...
                                    subtitle: Text(_selectedCompany ?? 'Sistema de Gestão de Estoque'),
                                  ),
                                  ListTile(
                                    leading: const Icon(Icons.store),
                                    title: const Text('Loja/Filial'),
                                    subtitle: Text(selectedStore?.name ?? 'Nenhuma loja selecionada'),
                                  ...
                                  ListTile(
                                    ...
                                    subtitle: Text(selectedStore?.address ?? 'Não informado'),
                                  ),
                                  if (_selectedWarehouse != null)
                                    ListTile(
                                      leading: const Icon(Icons.warehouse),
                                      title: const Text('Armazém'),
                                      subtitle: Text(_selectedWarehouse!),
                                    ),
                                ],
                              );
                            },
